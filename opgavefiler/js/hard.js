...
let emailInput = document.getElementById('myInput')
let emailButton = document.getElementById('submitBut')


emailButton.addEventListener('click', () => {
    let userEmail = validateEmail(emailInput.value)

    if (userEmail == true) {
        emailInput.classList.remove('falseInput')
        emailInput.classList.add('trueInput')
    } else {
        emailInput.classList.remove('trueInput')
        emailInput.classList.add('falseInput')
    }
})
...
const galleryContainer = document.getElementById('galleri')
let data = getData()
...
for (obj of data) {
    ...
    animalImg.src = obj.picture
    let animalTitle = document.createElement('h2')
    animalTitle.innerText = obj.name
    let animalShortDesc = document.createElement('h3')
    animalShortDesc.innerText = obj.shortDescription
    let animalDesc = document.createElement('p')
    animalDesc.innerText = obj.description

    galleryContainer.appendChild(container)
    container.appendChild(animalImg)
    container.appendChild(animalTitle)
    container.appendChild(animalShortDesc)
    container.appendChild(animalDesc)
    window.addEventListener('load', () => {
        container.classList.add('animalContainer')
